...
def dashboard(rows):
    ...
    ev = sum(1 for r in rows if "@" in r.get("email_investment","") or "@" in r.get("email_general",""))
...
def write_md(rows, dupes):
    ...
    with open(os.path.join(ROOT, "CALL_LISTS.md"), "w", encoding="utf-8") as f:
        ...
        for title, lst in call_lists(rows).items():
            ...
            for i, r in enumerate(lst, 1):
                dmn = r['decision_maker_name'] if r['decision_maker_name'] and r['decision_maker_name']!='NOT PUBLICLY IDENTIFIED' else 'Investment team'
                opening = f"Good morning, this is [Founder], founder of Vytalix, a UK health-technology group. I'm calling because {r['investor_name']} backs {(r['relevant_portfolio'].split(';')[0].split(',')[0] or 'early-stage health-tech')[:60]} and I'd value 20 minutes with {dmn} on a maternal-health platform we're developing. Who is the right person to speak to?"
                f.write(f"| {i} | {r['investor_name']} | {r['best_phone']} | {dmn} | {r['investor_type']} | {r['fit_score_100']} | {r['why_investor_for_vytalix'][:120]} | {opening} | {r['pitch_angle'][:120]} | Book a 20-min intro call; confirm stage/ticket fit; identify process | Email one-pager same day; follow-up call day 5; LinkedIn day 10 | {r['crm_status']} |\n")
            f.write("\n")
    ...
    with open(os.path.join(ROOT, "OUTREACH_TOP25.md"), "w", encoding="utf-8") as f:
        ...
        for i, r in enumerate(top, 1):
            dmx = r['decision_maker_name'] if r['decision_maker_name'] and r['decision_maker_name']!='NOT PUBLICLY IDENTIFIED' else None
            first = dmx.split()[0] if dmx else "there"
            port = (r['relevant_portfolio'].split(';')[0].split(',')[0]).strip() or "your health-tech portfolio"
            f.write(f"\n---\n\n## {i}. {r['investor_name']}\n\n**PHONE OPENING**\n\n\"Hello, my name is [Founder], founder of Vytalix, a UK health-technology group building MaternaLink, a maternal-health coordination platform in development for the UK and Nigeria. I'm calling {r['investor_name']} because of your work with {port}. I'm not asking for a decision today; I'd like 20 minutes with {dmx or 'the investment team'} to test whether this is a fit for your {r['investment_stage'] or 'early-stage'} mandate. Is {dmx or 'the investment team'} the right route, and what's the best way to get a slot?\"\n\n")
